	...
	<xsl:template match="/request">
		<div style="font-size:14px; width:100%;">
			<font style="font-size:14px;">
				<xsl:value-of select="//captions/page/@caption"/>:
				...
				<xsl:value-of select="concat(//captions/from/@caption,' ',//query/@maxpage)"/>
			</font>
		</div>
		<xsl:for-each select="//query/entry">
			<table width="100%" style="border-collapse:collapse" class="tblCorr{@docid}{@doctype}" level="1">
				<tr>
					<td style="text-align:left">
						<a href="" style="margin-left:3px" id="a{@docid}{@doctype}">
							<xsl:choose>
								<xsl:when test="@count = '0'">
									<xsl:attribute name='href'>javascript:collapsChapterCorr('<xsl:value-of select="@docid"/>','<xsl:value-of select="position()"/>','<xsl:value-of select="@url"/>', <xsl:value-of select="@doctype"/>,<xsl:value-of select="../@currentpage"/>)</xsl:attribute>
									<img border='0' src="/SharedResources/img/classic/1/minus.png" id="img{@categoryid}{@doctype}"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:attribute name='href'>javascript:openListRespDoc('<xsl:value-of select="@docid"/>',<xsl:value-of select="@doctype"/>,'<xsl:value-of select="position()"/>', <xsl:value-of select="../@currentpage"/>)</xsl:attribute>
									<img border='0' src="/SharedResources/img/classic/1/plus.png" id="img{@docid}{@doctype}"/>
								</xsl:otherwise>
							</xsl:choose>
						</a>
