						...
						<div class="button_panel">
							<span style="float:left">
								...
								<xsl:if test="document/actionbar/action[@id='NEW_DOCUMENT']/@mode = 'ON'">
									<button title ="{document/actionbar/action[@id='NEW_DOCUMENT']//@hint}" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="btnnewkr" style="margin-right:5px" autocomplete="off">
										<xsl:attribute name="onclick">javascript:window.location.href="Provider?type=edit&amp;id=essubtype&amp;key=&amp;parentdocid=<xsl:value-of select="document/@docid"/>&amp;parentdoctype=<xsl:value-of select="document/@doctype"/>"</xsl:attribute>
										<span>
											<img src="/SharedResources/img/classic/icons/page_white_add.png" class="button_img"/>
											<font class="button_text"><xsl:value-of select="document/actionbar/action[@id='NEW_DOCUMENT']/@caption"/></font>
										</span>
									</button>
								</xsl:if>
							</span>
