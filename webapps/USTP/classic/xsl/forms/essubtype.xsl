								...
								<form action="Provider" name="frm" method="post" id="frm" enctype="application/x-www-form-urlencoded">
									<div display="block" id="property">
										...
										<table width="100%" border="0">
											<tr>
												<td class="fc"><xsl:value-of select="document/captions/estype/@caption"/> :</td>
												<td>
													<xsl:variable name="estype" select="document/fields/estype/@attrval"/>
													<select size="1" name="estype" style="width:612px;" class="select_editable" autocomplete="off">
														<xsl:if test="$editmode !='edit'">
															<xsl:attribute name="disabled"/>
															<xsl:attribute name="class">select_noteditable</xsl:attribute>
															<option value="{document/fields/estype/@attrval}">
																<xsl:attribute name="selected">selected</xsl:attribute>
																<xsl:value-of select="document/fields/estype"/>
															</option>
														</xsl:if>
														<xsl:if test="$editmode ='edit'">
															<option value=" ">
														...
														<xsl:for-each select="document/glossaries/estype/query/entry">
															<option value="{@docid}">
																<xsl:if test="$estype = @docid">
																	<xsl:attribute name="selected">selected</xsl:attribute>
																</xsl:if>
																<xsl:if test="$estype = @docid">
																	<xsl:attribute name="selected">selected</xsl:attribute>
									...
									<input type="hidden" name="id" value="essubtype"/>
